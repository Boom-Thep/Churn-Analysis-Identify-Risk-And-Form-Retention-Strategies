...
--Classify customers into CLTV tiers to find high-value customers at risk. (USE)
SELECT 
  customer_status,
  CASE 
    WHEN cltv >= 5000 THEN 'High Value'
...
--Age group by cltv segment **Important** --> total count of customer in each segment both churn and not churn
SELECT 
   age_group,
...
-- --Age group by cltv segment **Important** --> total count of customer in each segment and churn rate.
SELECT 
   age_group,
   -- High Value Customers
   SUM(CASE WHEN cltv >= 5000 THEN 1 ELSE 0 END) AS high_value_count,
   ROUND(
      SUM(CASE WHEN cltv >= 5000 THEN 1 ELSE 0 END) 
      FILTER (WHERE customer_status = 'Churned') * 1.0 / 
      (SUM(CASE WHEN cltv >= 5000 THEN 1 ELSE 0 END)) * 100,2)
    AS high_value_churn_rate,

   -- Medium Value Customers
   SUM(CASE WHEN cltv BETWEEN 2500 AND 4999 THEN 1 ELSE 0 END) AS medium_value_count,
   ROUND(
      SUM(CASE WHEN cltv BETWEEN 2500 AND 4999 THEN 1 ELSE 0 END) 
      FILTER (WHERE customer_status = 'Churned') * 1.0 / 
      (SUM(CASE WHEN cltv BETWEEN 2500 AND 4999 THEN 1 ELSE 0 END)) * 100, 2)
   AS medium_value_churn_rate,

   -- Low Value Customers
   SUM(CASE WHEN cltv < 2500 THEN 1 ELSE 0 END) AS low_value_count,
   ROUND(
      SUM(CASE WHEN cltv < 2500 THEN 1 ELSE 0 END) 
      FILTER (WHERE customer_status = 'Churned') * 1.0 / 
      (SUM(CASE WHEN cltv < 2500 THEN 1 ELSE 0 END)) * 100, 2)
    AS low_value_churn_rate
